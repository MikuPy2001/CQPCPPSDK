超级前言->其实源码不在这里..请点击上面那个蓝色的CQCPPSDK或者[点我](https://coding.net/u/tatami/p/CQPCPPSDK/git/tree/master "点我")  
## 前言  
本项目由VS2017编写,如果您打算直接在本SDK项目的基础上编写,请确认兼容性。  
您也可以新建一个项目,然后将所有`src\CQSDK`文件夹中的文件导入到您的项目中.  
  
## 本SDK特点  
- 以 [测试版] CQP_SDK_V9_171115 为基础开发  
- 良好的事件包装机制  
- 良好的API包装机制  
...
最后将文件名改为 **[应用ID].JSON** 保存到您的项目文件夹备用.  
...
| 目录 | 说明 |  
| :- | :- |  
|CQSDK/|CQSDK头文件目录,以下代码依赖的头文件都在这里|  
|CQSDK/cpp/|CQSDK源码目录,用于实现头文件,必须与插件一起编译|  
|APPINFO.h|应用ID头文件|  
|appmain.cpp|插件入口,以及部分示例,可以删除|  
|main.cpp|DLL入口|  
|test_main.cpp|测试项目的入口|  
...
|180|撤回消息|deleteMsg|
标注为粗体红色的Api涉及帐号权限等安全问题，因此在使用时会向用户进行“敏感权限”确认。  
