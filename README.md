...
|CQSDKcpp/|CQSDK源码目录,用于实现头文件,通常情况无需使用|  
|APPINFO.h|应用ID头文件|  
...
| 目录 | 说明 |  
| :- | :- |
|SDK_Release.lib|实现了SDK的静态lib,Release模式必须包含|  
|SDK_Debug.lib|实现了SDK的静态lib,Debug模式必须包含|  
|CQP.lib|酷Q原始函数导入lib,用于SDK的编译|  
|CQP.lib.e|酷Q原始函数导入lib的易语言实现,如果有新增函数未添加,您可以使用此文件自行编译CQP.lib|  
